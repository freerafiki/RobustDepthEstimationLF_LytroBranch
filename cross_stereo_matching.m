%%%%%%%%%%%%%%%%%%%%%
%
% calculating the cost volume using two triplets of images
% horizontally and vertically
% at the moment not using the corner images
%
%
%
%%%%%%%%%%%%%%%%%%%%

function [cv, depthmap, conf] = cross_stereo_matching(EIs, c_img, ws, alpha, d_min, d_max, matte, gamma1, gamma2, priors, use_failure_prediction)

    fprintf('\nComputing cost volume from correspondences!\n')
    % computes disparity from correspondences using multi view approach
    tic
       
    fprintf('First Epipolar Line - Horizontal..\n');
    %% 1) compute cost cubes
    imgL = rgb2gray(uint8(EIs(:,:,:,5)));
    imgC = rgb2gray(uint8(EIs(:,:,:,4)));
    imgR = rgb2gray(uint8(EIs(:,:,:,3)));
    [cvL1, cvR1] = tri_stereo_v3(double(imgL), double(imgC), double(imgR), (d_min), (d_max), (ws), (alpha), 1);
    ...
    fprintf('Second Epipolar Line - Vertical..\n');
    
    imgL = rgb2gray(uint8(EIs(:,:,:,2)));
    imgR = rgb2gray(uint8(EIs(:,:,:,8)));
    [cvL2, cvR2] = tri_stereo_v3(double(imgL), double(imgC), double(imgR), (d_min), (d_max), (ws), (alpha), 2);
    imgL_s1 = impyramid(imgL, 'reduce');
    imgL_s2 = impyramid(imgL_s1, 'reduce');
    imgR_s1 = impyramid(imgR, 'reduce');
    imgR_s2 = impyramid(imgR_s1, 'reduce');
    ...
    cur_cubes = zeros(size(cvL1msc, 1), size(cvL1msc, 2), 4);
    ...
    for k = 1: size(cvL1msc,3)
        ...
        cur_cubes(:,:,3) = cvR1msc(:,:,k);
        cur_cubes(:,:,4) = cvR2msc(:,:,k);
    ...
    depths = zeros(size(depth_L1_wta, 1), size(depth_L1_wta,2), 4 );
    depths(:,:,1) = depth_L1_wta;
    depths(:,:,2) = depth_L2_wta;
    depths(:,:,3) = depth_R1_wta;
    depths(:,:,4) = depth_R2_wta;
